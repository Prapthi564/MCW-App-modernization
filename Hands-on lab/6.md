...
1. Once the installation is **Finished (1)** select **Restore (2)** to download dependencies for the project.
   ...
   > **Note:** If the C# extension is already present in the VS code, it may not ask you to restore it.
...
1. Select **Deploy**.
   ...
   > **Note:** If your Deploy to app is getting failed due to that path does not exists then create a folder with name **bin\Release\netcoreapp3.1\publish** and try to deploy again.
...
1. Switch to **Environment variables (1)** Blade, select **Application settings (2)** and  click on **Add (3)**.
